...
#include <algorithm/ulspose/utilities/cuda.hpp>
#include <algorithm/ulspose/utilities/errorAndLog.hpp>
#include <algorithm/ulspose/core/netCaffe.hpp>
...
namespace pose
{
    ...
    NetCaffe::NetCaffe(const std::array<int, 4>& netInputSize4D, std::istream* caffeProto, std::istream* caffeTrainedModel, const int gpuId, const std::string& lastBlobName) :
    mGpuId{gpuId},
    // mNetInputSize4D{netInputSize4D}, // This line crashes on some devices with old G++
    mNetInputSize4D{netInputSize4D[0], netInputSize4D[1], netInputSize4D[2], netInputSize4D[3]},
    mNetInputMemory{std::accumulate(mNetInputSize4D.begin(), mNetInputSize4D.end(), 1, std::multiplies<int>()) * sizeof(float)},
    mCaffeProto_stm{caffeProto},
    mCaffeTrainedModel_stm{caffeTrainedModel},
    mLastBlobName{lastBlobName}
    {
    }
    ...
    void NetCaffe::initializationOnThread()
    {
        try
        {
            ...
            if (mCaffeProto.empty()) {
                upCaffeNet.reset(new caffe::Net<float>{mCaffeProto_stm, caffe::TEST});
                upCaffeNet->CopyTrainedLayersFromMemory(mCaffeTrainedModel_stm);
            } else {
    ...
    boost::shared_ptr<caffe::Blob<float>> NetCaffe::blob_by_name(const std::string& blob_name) const
    {
        try
        {
            boost::shared_ptr<caffe::Blob<float>> spFetchBlob;
            spFetchBlob = upCaffeNet->blob_by_name(blob_name);
            if (spFetchBlob == nullptr)
                error("The output blob is a nullptr. Did you use the same name than the prototxt? (Used: " + blob_name   + ").", __LINE__, __FUNCTION__, __FILE__);
            cudaCheck(__LINE__, __FUNCTION__, __FILE__);
            return spOutputBlob;
        }
        catch (const std::exception& e)
