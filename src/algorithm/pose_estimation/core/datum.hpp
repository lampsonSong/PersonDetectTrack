...
namespace pose
{
    ...
    struct Datum
    {
         ...
         * It consists of a blending of the inputNetData and the algorithm/ulspose/pose/body part(s) heatmap/PAF(s).
         ...
         * Order heatmaps: body parts + background (as appears in POSE_BODY_PART_MAPPING) + (x,y) channel of each PAF (sorted as appears in POSE_BODY_PART_PAIRS). See `algorithm/ulspose/pose/poseParameters.hpp`.
        ...
        std::pair<int, std::string> elementRendered; /**< Pair with the element key id POSE_BODY_PART_MAPPING on `algorithm/ulspose/pose/poseParameters.hpp` and its mapped value (e.g. 1 and "Neck"). */
